...
{
  const tableSql = db
    .prepare("SELECT sql FROM sqlite_master WHERE type='table' AND name='pedidos'")
    .get();
  ...
  const seedItems = [
    {
      ...
      descripcion: 'Queso provolone gratinado con orégano y tomate',
      precio: 1800,
      cat: 1
    },
    {
    ...
    },
    {
      ...
      descripcion: 'Jamón, salame, queso y aceitunas',
      precio: 2200,
      cat: 1
    },
    {
      nombre: 'Bife de chorizo',
      descripcion: '350g de bife madurado a la parrilla',
      precio: 4500,
      cat: 2
    },
    {
      nombre: 'Milanesa napolitana',
      descripcion: 'Milanesa de ternera con salsa, jamón y mozzarella',
      precio: 3800,
      cat: 2
    },
    {
      nombre: 'Pasta del día',
      descripcion: 'Preguntá al mozo por la pasta y salsa del día',
      precio: 2900,
      cat: 2
    },
    {
      nombre: 'Pollo a la plancha',
      descripcion: 'Pechuga de pollo con limón y papas fritas',
      precio: 3200,
      cat: 2
    },
    { nombre: 'Coca-Cola / Pepsi (500ml)', descripcion: null, precio: 900, cat: 3 },
    {
      nombre: 'Cerveza (porrón)',
      descripcion: 'Quilmes / Heineken / Stella Artois',
      precio: 1100,
      cat: 3
    },
    { nombre: 'Agua mineral (500ml)', descripcion: 'Con o sin gas', precio: 700, cat: 3 },
    {
      ...
      descripcion: 'Flan casero con crema y dulce de leche',
      precio: 1200,
      cat: 4
    },
    {
      nombre: 'Helado (2 bochas)',
      descripcion: 'Preguntá por los sabores disponibles',
      precio: 1100,
      cat: 4
    }
  ];
...
if (totalMods === 0) {
  ...
  const insertarTodos = db.transaction(() => {
    ...
    const getMod = (nombre) =>
      db.prepare('SELECT id FROM modificadores WHERE nombre = ?').get(nombre)?.id;
    const getItem = (nombre) =>
      db.prepare('SELECT id FROM menu_items WHERE nombre = ?').get(nombre)?.id;
...
// ============================================================
// USUARIOS (tabla y seed — solo se crea si no existe)
// ============================================================
...
if (totalUsuarios === 0) {
  ...
  const usuariosSeed = [
    { username: 'DiegoU', password: 'DU12345678' },
    { username: 'MartinC', password: 'MC12345678' },
    { username: 'MartinS', password: 'MS12345678' },
    { username: 'ClaudiaM', password: 'CM12345678' },
    { username: 'FranciscaH', password: 'FH12345678' }
  ];

  const ins = db.prepare('INSERT INTO usuarios (username, password_hash) VALUES (?, ?)');

  // bcryptjs.hashSync permite hacerlo de forma síncrona (necesario en este contexto síncrono)
  usuariosSeed.forEach(({ username, password }) => {
    const hash = bcrypt.hashSync(password, SALT_ROUNDS);
    ins.run(username, hash);
  });

  console.log('[DB] ✅ 5 usuarios creados.');
}
